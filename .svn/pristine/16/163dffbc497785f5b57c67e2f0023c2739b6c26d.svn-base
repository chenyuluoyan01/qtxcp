    ...
    <div>
        <div class="i-form">
            ...
            <Form :ref="formData.rel" :model="formData.rel" :label-width="120">
                ...
                <FormItem label="态度评分">
                   <span class="star">
                       <span class="mask" :style="{width:70-appraiseDtlList[0]*13.5+'px'}"></span>
                       <Icon type="android-star" size="17" color="red" v-for = 'i in appraiseDtlListInt[0]' :key = 'i'></Icon>
                   </span>
                   <span>{{appraiseDtlList[0]}}</span>
                </FormItem>
                <FormItem label="服务评分">
                   <span class="star">
                       <span class="mask" :style="{width:70-appraiseDtlList[1]*13.5+'px'}"></span>
                        <Icon type="android-star" size="17" color="red" v-for = 'i in appraiseDtlListInt[1]' :key = 'i'></Icon>  
                   </span>                 
                   <span>{{appraiseDtlList[1]}}</span>
                </FormItem>
                <FormItem label="店面环境">
                   <span class="star">
                       <span class="mask" :style="{width:70-appraiseDtlList[2]*13.5+'px'}"></span>
                       <Icon type="android-star" size="17" color="red" v-for = 'i in appraiseDtlListInt[2]' :key = 'i'></Icon>
                   </span>
    ...
    methods: {
        ...
        getPromise: function(){
            ...
            this.get(Api.appraise.getById,{appraiseId:d.appraiseId},res=>{
                ...
                this.appraiseDtlList.map((item)=>{
                    this.appraiseDtlListInt.push(Math.ceil(item))
                })
            })
        }
    }
