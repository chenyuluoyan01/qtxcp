    ...
    <div>
        <div class="i-form">
            <Form :ref="formData.rel" :model="formData.rel" :label-width="120">
                ...
                <FormItem label="活动图片：" class="formMust" v-if='formData.imgUpConfig.status'>
                    <upload
...
export default {
    ...
    methods: {
        handleInit(){
            ...
            //特殊字段设置
            this.formData.rel.distributedTime = this.utils.formatDate(new Date(this.formData.rel.distributedTime),"yyyy-MM-dd");
        ...
        save(){
            ...
            this.utils.resolveForm(_this,()=>{
                //是否有时间改动
                if(this.formData.rel.distributedTime.search(':')==-1){
                    this.formData.rel.distributedTime += ' 00:00:00';
                };
                this.post(Api.banner.update,this.formData.rel,(res)=>{
