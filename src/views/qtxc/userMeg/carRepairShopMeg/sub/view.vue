        ...
        <div class="i-form">
            <Form :ref="formData.rel" :model="formData.rel" :label-width="120">
                ...
                <FormItem label="维修厂图片：" class="formMust">
                    <upload v-if='formData.imgUpConfig.init' :count="formData.imgUpConfig.count" :defaultList="formData.imgUpConfig.defaultList" :fileDir='formData.imgUpConfig.fileDir' @upList="getUpList"></upload>
                ...
                <FormItem label="店铺LOGO：" class="formMust">
                    <upload v-if='formData.imgUpConfigL.init' :count="formData.imgUpConfigL.count" :defaultList="formData.imgUpConfigL.defaultList" :fileDir='formData.imgUpConfigL.fileDir' @upList="getUpListL"></upload>
                ...
                <FormItem label="品牌倾向：" class="formMust">
                    ...
                    <Select @on-change='onSelect' v-model="formData.rel.brandIds" filterable multiple style="width:520px">
                      <Option  v-for='(todo,i) in single.formData.brandList.list' :value="todo.brandId">{{todo.brandName}}</Option>
                    </Select>
                    <span v-if='formData.initF&&formData.rel.brandIds==""' class="error-mark">
                        <Icon type="close-circled"></Icon> 必填
                    </span>
                </FormItem>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      let datas = {};
      datas.factoryId = data.factoryId;
      datas.factoryName = data.factoryName;
      this.utils.resolveForm(_this, () => {
        this.post(Api.factory.modify,datas,res => {
            console.log(res);
            this.$Message.success("修改汽修厂成功！");
            // this.refshData(_this.$parent);
        });
      });
    },
    addSel: function() {
