            ...
            <table class="table table-hover">
                ...
                <tbody>
                    <tr v-for = "(item,index) in list" :key='index+"xc"'>
                        ...
                        <td>{{item.nickName}}</td>
                        <td>{{item.phone}}</td>
                        <td>{{item.userStatus == "0"?"正常":"冻结"}}</td>
                        <td>
                            <span class="btn" @click='view(item)'>查看</span>
                            <span class="btn" @click='update(item)'>编辑</span>
                            <freeze @freeze='freeze(item)' v-if='item.userStatus=="0"'></freeze>
                            <span class="btn" v-else @click='freeze(item,"1")'>解冻</span>
                            <!-- <cut @del='del'></cut> -->
                        </td>
                    </tr>
