            ...
            <Tabs type="card">
                <TabPane label="搭电">
                    <div class="padding-middle">
                        单次救援价格： <InputNumber :max="10000" :min="1" :step="1" v-model="powerPrice" :formatter="value => `${value}`"
            :parser="value => Math.ceil(value)"></InputNumber> 元
                        <Button type='primary' size='small' style="margin-left:40px;" @click="setda">确定</Button>
                    </div>
                </TabPane>
                <TabPane label="拖车">
                    <div class="padding-middle">
                        公里计算： <InputNumber :max="10000" :min="1" :step="1" v-model="minimumKm" :formatter="value => `${value}`"
            :parser="value => Math.ceil(value)"></InputNumber> km内
                        <InputNumber :max="10000" :min="1" :step="1" v-model="priceMinimumKm" :formatter="value => `${value}`"
            :parser="value => Math.ceil(value)"></InputNumber> 元/次，
                        超出部分按 <InputNumber :max="10000" :min="1" :step="1" v-model="pricePerKm" :formatter="value => `${value}`"
            :parser="value => Math.ceil(value)"></InputNumber> 元/公里计算
                        <Button type='primary' size='small' style="margin-left:40px;" @click="settuo">确定</Button>
                    </div>
                </TabPane>
                <TabPane label="换胎">
                    <div class="padding-middle">
                        单次换胎价格： <InputNumber :max="10000" :min="1" :step="1" v-model="changeTyre" :formatter="value => `${value}`"
            :parser="value => Math.ceil(value)"></InputNumber> 元
...
export default {
    ...
    methods: {
        ...
        settuo:function() {
            this.post(Api.server.update,{minimumKm:this.minimumKm,priceMinimumKm:this.priceMinimumKm,pricePerKm:this.pricePerKm,serviceItemId:this.serviceItemId},(res)=>{
                this.$Message.success("设置成功！")
            })
        },
        sethuan:function() {
